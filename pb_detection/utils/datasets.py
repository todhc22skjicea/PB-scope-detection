...
class LoadImages:  # for inference
    ...
    def histogram_equalization_16bit(self,img):  
      hist, bins = np.histogram(img.flatten(), bins=range(65536))  
      cdf = hist.cumsum()  
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            self.count += 1
            img0 = cv2.imread(path,cv2.IMREAD_ANYDEPTH) # BGR
            if img0.dtype == 'uint8':
              ...
              img1 = cv2.imread(path,cv2.IMREAD_ANYDEPTH)  # BGR
              print(img1.max())

              [img_x,img_y] = img1.shape
              min_img1 = np.min(img1)
              max_img1 = np.max(img1)
              img1 =  np.array(np.rint(255*((img1 - min_img1)/(max_img1- min_img1))), dtype=np.uint8)
              img0 = np.zeros([img_x,img_y,3])
              img0[:,:,0] = img1
              img0[:,:,1] = img1
              img0[:,:,2] = img1
              print(img0[:,:,2].max())
